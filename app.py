...
def like():
    ...
    # Parse UIDs: either comma-separated or range (e.g., "1001-1005")
    uids = []
    if '-' in uid_input:
        try:
        ...
        uids = [uid.strip() for uid in uids if uid.strip().isdigit()]
    
    if not uids:
        return jsonify(error="No valid UIDs provided"), 400
    
    # Process all UIDs concurrently
    urls = URLS_LIKE
    results = asyncio.run(asyncio.gather(*[process_uid(uid, server, urls) for uid in uids]))
